...
//putting fetch posts directly as second arg, could also write a mapDispatchToProps function
//mapping dispatch to props allows us to pass in FUNCTIONS, in this case we use it to allow fetchposts function to be a prop 
//that's the point, when we want to interact with store, need to dispatch, now we can via a prop!
//point is to be able to call this.props.fetchPosts when component mounts in order to fetch our data for display

//when there's no second arg, you are provided the ability to use a disptch prop by connect
